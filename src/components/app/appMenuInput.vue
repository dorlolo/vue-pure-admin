...
import { getMenuDropDownList } from "@/api/appMenu";
...
async function updateMenuDropDownList(forceUpdate?: boolean) {
  if (forceUpdate || restaurants.value.length == 0) {
    const res = await getMenuDropDownList({});
    if (res.list) {
      restaurants.value = [];
      restaurants.value.push({ value: "无", link: "0" });
      res.list.forEach(v => {
        restaurants.value.push({ value: v.name, link: String(v.id) });
      });
    }
  }
}

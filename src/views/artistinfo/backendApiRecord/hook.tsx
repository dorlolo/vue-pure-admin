...
import {
  BackendOperationRecordType,
  deleteBackendOperationRecord,
  updateBackendOperationRecord,
  createBackendOperationRecord,
  getBackendOperationRecordList,
  getBackendOperationRecordDetail
} from "@/api/fonchainArtistinfoBackendOperationRecordApi";
//-----------------------------搜索表单-----------------------------------
...
export const tableData = ref<BackendOperationRecordType[]>([]);
...
export const formRef = ref<{
  ...
  dialogTitle: string;
  operation: string;
  formData: BackendOperationRecordType;
  list: Array<any>;
  rules: any;
}>({
  ////展示表单
...
export function useSearchForm() {
  ...
  async function onSearch() {
    ...
    const response = await getBackendOperationRecordList({
...
export function useDataTable() {
  ...
  function onOpenDeleteDialog(
    row: BackendOperationRecordType,
  ...
  async function onDelete() {
    ...
    switch (tableDeleteRef.value.deleteMode) {
      case "single":
        ...
        res = await deleteBackendOperationRecord({
...
export function useEditForm() {
          ...
          case "create":
            {
              ...
              const res = await createBackendOperationRecord(
                formRef.value.formData
              );
              if (res.code === 0) {
          ...
          case "edit":
            {
              ...
              const res = await updateBackendOperationRecord(
                formRef.value.formData
              );
              if (res.code === 0) {
  ...
  function onEdit(row: BackendOperationRecordType) {
    formRef.value.formData = row;
    onOpen("edit");
  }

  async function onDetail(row: BackendOperationRecordType) {
    const res = await getBackendOperationRecordDetail({ id: row.id });
    if (HandleResponseCode(res as baseResponse)) {
      // & BackendOperationRecordType
      if (res.data.body !== "") {
        res.data.body = JSON.stringify(JSON.parse(res.data.body), null, 2); // 缩进 2 个空格
      }
      if (res.data.result !== "") {
        res.data.result = JSON.stringify(JSON.parse(res.data.result), null, 2); // 缩进 2 个空格
      }

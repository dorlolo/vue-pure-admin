...
export const tableData = ref<MemoryCardType[]>([]);
...
export function useSearchForm() {
  ...
  async function onSearch() {
    ...
    const response = await getMemoryCardList({
  ...
  async function onDelete() {
    ...
    switch (tableDeleteRef.value.deleteMode) {
      case "single":
        ...
        res = await deleteMemoryCard({ id: tableDeleteRef.value.deleteId });
        break;
      case "mult":
        res = await deleteMemoryCard({ ids: muiltSelectList.value });
        break;
      default:
        break;
    }
...
export function useEditForm() {
          ...
          case "create":
            {
              ...
              const res = await createMemoryCard(formRef.value.formData);
              if (res.code === 0) {
          ...
          case "edit":
            {
              ...
              const res = await updateMemoryCard(formRef.value.formData);
              if (res.code === 0) {
  ...
  function onEdit(row: MemoryCardType) {
    formRef.value.formData = row;
    onOpen("edit");
  }
  async function onDetail(row: MemoryCardType) {
    const res = await getMemoryCardDetail({ id: row.id });
    if (HandleResponseCode(res as baseResponse)) {
      formRef.value.formData = res.data;
      formRef.value.frontMarkdown = marked(res.data.front);
      formRef.value.backMarkdown = marked(res.data.back);
      onOpen("detail");

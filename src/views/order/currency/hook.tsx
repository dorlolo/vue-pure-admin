...
export const tableData = ref<CurrencyType[]>([]);
  ...
  async function onDelete() {
    ...
    switch (tableDeleteRef.value.deleteMode) {
      case "single":
        ...
        res = await deleteCurrency({ id: tableDeleteRef.value.deleteId });
        break;
      case "mult":
        res = await deleteCurrency({ ids: multipleSelectList.value });
        break;
      default:
        break;
    }
...
export function useEditForm() {
  ...
  async function onSubmit(formEl: FormInstance | undefined) {
    ...
    console.log("formEl", formEl === undefined);
    if (!formEl) return;
          ...
          case "create":
            {
              ...
              const res = await createCurrency(formRef.value.formData);
              if (res.code === 0) {
          ...
          case "edit":
            {
              ...
              const res = await updateCurrency(formRef.value.formData);
              if (res.code === 0) {
  ...
  function onEdit(row: CurrencyType) {
    formRef.value.formData = row;
    onOpen("edit");
  }

  async function onDetail(row: CurrencyType) {
    const res = await getCurrencyDetail({ id: row.id });

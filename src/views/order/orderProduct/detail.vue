...
import { formRef, useEditForm, useDetail, RestaurantItem } from "./hook";
...
import ProductPackageGroupTable from "./components/productGroupTable.vue";
import ProductSalesForm from "./components/productSalesForm.vue";
defineOptions({
...
initToDetail();
onMounted(async () => {
  formData.value.detail.id = Number(getParameter.id);
  freshData();
});

async function freshData() {
  const res = await getProductDetail({ id: getParameter.id });
  ...
  <div class="detail">
    ...
    >
      ...
      <el-tab-pane label="定价" name="second">
        <productSalesForm
          :productId="Number(getParameter.id)"
          :productType="formData.detail.productType"
        />
      </el-tab-pane>
      ...
      <el-tab-pane
        label="套餐"
        name="package"
        v-if="formData.detail.productType == 2"
        style="height: 100%"
      >
        <ProductPackageGroupTable
          :productId="formData.detail.id"
    ...
    <el-dialog v-model="dialogForm.dialogVisible" :before-close="onCloseDialog">
      ...
      <template v-slot:footer>
        <el-button
          ...
          v-if="dialogForm.mTagDetail.id !== 0"
          >更 新</el-button
        >

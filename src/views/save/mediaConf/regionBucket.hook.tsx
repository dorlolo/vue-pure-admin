...
import {
  MediaOssRegionType,
  deleteMediaOssRegion,
  updateMediaOssRegion,
  createMediaOssRegion,
  getMediaOssRegionList,
  getMediaOssRegionDetail
} from "@/api/mediaOssRegionApi";
//-----------------------------搜索表单-----------------------------------
...
export const tableData = ref<MediaOssRegionType[]>([]);
...
export function useSearchForm() {
  ...
  async function onSearch() {
    ...
    const response = await getMediaOssRegionList({
      page: tablePagination.currentPage,
      pageSize: tablePagination.pageSize,
      ...searchFormData
    });
    console.log("onSearch1111111111111111", response);
    if (response.code != 0) {
  ...
  async function onDelete() {
    ...
    switch (tableDeleteRef.value.deleteMode) {
      case "single":
        ...
        res = await deleteMediaOssRegion({ id: tableDeleteRef.value.deleteId });
        break;
      case "mult":
        res = await deleteMediaOssRegion({ ids: multipleSelectList.value });
        break;
      default:
        break;
    }
...
export function useEditForm() {
          ...
          case "create":
            {
              ...
              const res = await createMediaOssRegion(formRef.value.formData);
              if (res.code === 0) {
          ...
          case "edit":
            {
              ...
              const res = await updateMediaOssRegion(formRef.value.formData);
              if (res.code === 0) {
  ...
  function onEdit(row: MediaOssRegionType) {
    formRef.value.formData = row;
    onOpen("edit");
  }

  async function onDetail(row: MediaOssRegionType) {
    const res = await getMediaOssRegionDetail({ id: row.id });
